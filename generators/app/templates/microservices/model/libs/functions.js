...
function Post(options, Data, done, type = null) //add or edit users
{
    ...
    if (options['cookie']) {
        [opt_Edit.headers.cookie, opt_Edit.headers['Content-Type'], opt_Edit.withCredentials, options['cookie']] = [options['cookie'], 'application/json', true, null];
        ...
        request = http.get(link, function(response) {
            response.on("data", function(chunk) {
                ...
                switch (toReplace instanceof Array) {
                    case true:
                        toReplace.forEach(v => buffer = buffer.replace(v, ""));
                    case false:
                        buffer = buffer.replace(toReplace, "");
                }
            });
            response.on("end", function(err) {
                transmission.Response = JSON.parse(buffer);
                done(err, transmission);
            });
        })
}
module.exports = {
